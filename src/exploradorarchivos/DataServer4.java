...
public class DataServer4 extends UnicastRemoteObject implements DataServer {

    public DataServer4(File directorio) throws RemoteException {
        super();
    ...
    public static void main(String args[]) {
        try {
            directorio = new File("./Data/DataServer4");
            if (!directorio.exists()) {
                try {
                    directorio.mkdirs();
                } catch (Exception e) {}
            }
            reg = LocateRegistry.createRegistry(1106);
            reg.rebind("DataServer4", new DataServer1(directorio));
            System.out.println("DataServer4 conectado");
        } catch (Exception e) {
